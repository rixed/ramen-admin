...
void TimeLineView::updateOrCreateTimeLine(FunctionItem const *functionItem) {
  ...
  if (!archivedTimes || archivedTimes->empty()) {
    if (verbose)
      qDebug() << "TimeLineView: Added function" << name << "has no archives";
    return;
  }
  ...
  for (i = 0; i < labels.count(); i++) {
    ...
    if (c == 0) {
      ...
      if (verbose)
        qDebug() << "TimeLineView: Have to insert" << name << "before"
                 << labels[i];
      break;
    }
  }
  ...
  BinaryHeatLine *heatLine{
      new BinaryHeatLine((*archivedTimes)[0].since,
                         (*archivedTimes)[archivedTimes->size() - 1].until)};

  heatLine->setArchivedTimes(*archivedTimes);

  if (verbose)
    qDebug() << "TimeLineView: Adding a heatline for" << name << "starting at"
             << stringOfDate((*archivedTimes)[0].since) << "at row" << i << "/"
             << formLayout->rowCount();
...
void TimeLineView::removeTimeLine(FunctionItem const *functionItem) {
  ...
  for (int i = 0; i < labels.count(); i++) {
    ...
    if (verbose) qDebug() << "TimeLineView: Removing function" << name;
    BinaryHeatLine *heatLine{static_cast<BinaryHeatLine *>(
        formLayout->itemAt(i + 1, QFormLayout::FieldRole)->widget())};
...
void TimeLineView::highlightRange(QString const &label,
                                  QPair<qreal, qreal> const range) {
  for (int i = 0; i < labels.count(); i++) {
    if (labels[i] != label) continue;

    if (verbose) qDebug() << "TimeLineView: Highlighting function" << label;
    BinaryHeatLine *heatLine{static_cast<BinaryHeatLine *>(
        formLayout->itemAt(i + 1, QFormLayout::FieldRole)->widget())};

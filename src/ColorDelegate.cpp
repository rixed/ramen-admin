...
void ColorDelegate::paint(QPainter *painter, QStyleOptionViewItem const &option,
                          QModelIndex const &index) const {
  QColor c(index.data().value<QColor>());
  painter->fillRect(option.rect, c);
}

QSize ColorDelegate::sizeHint(QStyleOptionViewItem const &,
...
void ColorDelegate::setEditorData(QWidget *editor_,
                                  QModelIndex const &index) const {
  QColor c(index.data().value<QColor>());

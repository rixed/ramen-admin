...
static int getFieldNum(
  std::shared_ptr<Function const> func, std::string const &fieldName)
{
  ...
  qWarning()
    << "TimeChart: Cannot find field number for" << QString::fromStdString(fieldName);
  return -1;
}
...
void TimeChart::paintEvent(QPaintEvent *event)
{
  ...
  for (auto &it : funcs) {
    ...
    res.func->iterValues(m_viewPort.first, m_viewPort.second, true, res.columns,
      [&res](double time,
             std::vector<std::shared_ptr<dessser::gen::raql_value::t const>> const values)
      {

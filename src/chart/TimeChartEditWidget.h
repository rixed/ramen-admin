...
/* FIXME: the actual AtomicWidget tracking the key is already the
 * DashboardWidgetChart, which is one of the personalities of the polymorphic
 * DashboardWidget. Maybe this TimechartWidget should then not be an
 * AtomicWidget? */
class TimeChartEditWidget : public AtomicWidget {
 ...
 public:
  TimeChartEditWidget(QPushButton *submitButton, QPushButton *cancelButton,
                      QPushButton *deleteButton, QWidget *parent = nullptr);
  ...
  void iterFields(
      std::function<void(std::string const &site, std::string const &program,
                         ...
                         dessser::gen::dashboard_widget::field const &)>) const;

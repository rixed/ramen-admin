...
void FactorsDelegate::paint(QPainter *painter,
                            QStyleOptionViewItem const &option,
                            QModelIndex const &index) const {
  painter->fillRect(option.rect, painter->background());

  QPixmap pixmap(qvariant_cast<QPixmap>(index.data()));
  int const x(option.rect.x() + (option.rect.width() - pixmap.width()) / 2);
  int const y(option.rect.y() + (option.rect.height() - pixmap.height()) / 2);
  painter->drawPixmap(x, y, pixmap);
}

QSize FactorsDelegate::sizeHint(QStyleOptionViewItem const &,
                                QModelIndex const &index) const {
  QPixmap pixmap(qvariant_cast<QPixmap>(index.data()));
  return pixmap.size();
}

QWidget *FactorsDelegate::createEditor(QWidget *parent,
...
void FactorsDelegate::setEditorData(QWidget *editor_,
                                    QModelIndex const &index) const {
...
void FactorsDelegate::updateEditorGeometry(QWidget *editor,
                                           QStyleOptionViewItem const &,
                                           QModelIndex const &) const {

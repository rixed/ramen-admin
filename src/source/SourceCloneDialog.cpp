  ...
  : QDialog(parent)
{
  ...
  if (value) {
    QLabel *label {
      new QLabel(tr("Where to clone %1 into?")
    ...
    connect(cloneButton, &QPushButton::clicked,
            this, &SourceCloneDialog::cloneSource);
    connect(cancelButton, &QPushButton::clicked,
            this, &QDialog::reject);
    connect(newKeyEdit, &QLineEdit::textChanged,
            this, &SourceCloneDialog::validate);
    ...
    cloneButton = nullptr;
    QLabel *label {
      new QLabel(tr("Source for %1 has no value!?")
                 .arg(QString::fromStdString(srcPath))) };
    layout->addWidget(label);
  }
...
void SourceCloneDialog::cloneSource()
{
  ...
  if (verbose)
    qDebug() << "Saving cloned value into" << *newKey;

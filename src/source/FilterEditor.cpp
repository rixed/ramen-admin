...
FilterEditor::FilterEditor(QWidget *parent) : QWidget(parent) {
  ...
  connect(lhsEdit, &QLineEdit::textChanged, this, &FilterEditor::inputChanged);
  connect(opEdit, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &FilterEditor::inputChanged);
  connect(rhsEdit, &QLineEdit::textChanged, this, &FilterEditor::inputChanged);
}

bool FilterEditor::isEmpty() const {
  return lhsEdit->text().isEmpty() && rhsEdit->text().isEmpty();
}
...
void FilterEditor::setFunction(QModelIndex const &root) {
  if (verbose) qDebug() << "FilterEditor: set new completer";
...
QString const FilterEditor::description(QString const &prefix,
  ...
  return prefix + (lhs.isEmpty() ? "…" : lhs) + opDescription(op) +

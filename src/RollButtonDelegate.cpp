...
void RollButtonDelegate::paint(QPainter *painter,
  ...
  int const x{option.rect.x() + (option.rect.width() - pixmaps[n].width()) / 2};
  int const y{option.rect.y() +
              (option.rect.height() - pixmaps[n].height()) / 2};
  painter->drawPixmap(x, y, pixmaps[n]);
}

QSize RollButtonDelegate::sizeHint(QStyleOptionViewItem const &,
                                   QModelIndex const &) const {
  return minSize;
}

QWidget *RollButtonDelegate::createEditor(QWidget *parent,
                                          QStyleOptionViewItem const &,
                                          QModelIndex const &) const {
  IconSelector *editor{new IconSelector(pixmaps, parent)};
  editor->setWindowFlags(Qt::Popup);
  connect(editor, &IconSelector::selectionChanged, this,
          &RollButtonDelegate::commitAndCloseEditor);
  return editor;
}

void RollButtonDelegate::updateEditorGeometry(QWidget *editor,
                                              QStyleOptionViewItem const &,
                                              QModelIndex const &) const {
...
void RollButtonDelegate::setEditorData(QWidget *editor_,
                                       QModelIndex const &index) const {
  int const n{index.data().toInt()};
  Q_ASSERT(n < pixmaps.count());

  IconSelector *editor{static_cast<IconSelector *>(editor_)};
  if (verbose) qDebug() << "RollButtonDelegate: setSelected to" << n;
  editor->setSelected(n);
}

void RollButtonDelegate::setModelData(QWidget *editor_,

...
LogModel::LogModel(QObject *parent)
    : QAbstractListModel(parent), first(0), removingFirst(false) {
  logs.reserve(LOG_HISTORY);
}

QVariant LogModel::data(const QModelIndex &index, int role) const {
  if (index.row() >= (removingFirst ? logs.count() - 1 : logs.count()))
    return QVariant();

  if (index.column() >= 3) return QVariant();

  int const i = (index.row() + first + (removingFirst ? 1 : 0)) % logs.count();
...
void LogModel::append(QtMsgType type, QString const &msg_) {
  ...
  QString const msg(msg_.length() < maxLen ? msg_
                                           : msg_.left(maxLen - 1) + "…");

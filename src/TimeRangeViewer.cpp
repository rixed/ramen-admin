...
TimeRangeViewer::TimeRangeViewer(QWidget *parent) : AtomicWidget(parent) {
...
bool TimeRangeViewer::setValue(
  ...
  if (v->index() == dessser::gen::sync_value::TimeRange) {
    ...
    for (unsigned i = 0; i < sz; i++) {
      ...
      QTableWidgetItem *since{new QTableWidgetItem(stringOfDate(r.since))};
      QTableWidgetItem *until{new QTableWidgetItem(
          stringOfDate(r.until) + (r.growing ? QString("…") : QString()))};

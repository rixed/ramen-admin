...
ConfTreeModel::ConfTreeModel(QObject *parent) : QAbstractItemModel(parent) {
  ...
  connect(kvs.get(), &KVStore::keyChanged, this, &ConfTreeModel::onChange);
}

ConfTreeModel::~ConfTreeModel() { delete root; }

void ConfTreeModel::dump() const { root->dump(); }

QModelIndex ConfTreeModel::index(int row, int column,
                                 QModelIndex const &parent) const {
  ConfSubTree *parentTree{
      parent.isValid() ? static_cast<ConfSubTree *>(parent.internalPointer())
                       : root};
...
int ConfTreeModel::columnCount(QModelIndex const &) const { return 1; }
...
QModelIndex ConfTreeModel::find(std::string const &path) const {
  ...
  do {
    ...
    for (int i = 0; i < parent->count(); i++) {
      ...
      if (cmp < 0) {
        if (verbose)
          qDebug() << "ConfTreeModel: Cannot find"
...
ConfSubTree *ConfTreeModel::findOrCreate(ConfSubTree *parent,
                                         QStringList &names,
  ...
  if (verbose)
    qDebug() << "ConfTreeModel: inserting" << name << "with termValue"
             << (is_term ? termValue : "none (non-term)");

  QModelIndex parentIndex =
      parent == root ? QModelIndex()

...
void LogLine::paint(QPainter *painter, QStyleOptionViewItem const &option,
  ...
  QVariant const c{
      color(static_cast<QtMsgType>(index.data(Qt::UserRole).toInt()))};
  if (c.canConvert<QColor>()) painter->setPen(c.value<QColor>());

  painter->drawText(option.rect, Qt::AlignLeft | Qt::TextWordWrap, text);
  painter->restore();
}

QSize LogLine::sizeHint(QStyleOptionViewItem const &option,
                        const QModelIndex &index) const {
...
LoggerView::LoggerView(QWidget *parent)
  ...
  connect(scrollCheckBox, &QCheckBox::stateChanged, this,
          &LoggerView::setDoScroll);
  connect(saveButton, &QPushButton::clicked, this, &LoggerView::pickSaveFile);
}

LoggerView::~LoggerView() { flush(); }
...
void LoggerView::setModel(QAbstractItemModel *model) {
  ...
  connect(model, &LogModel::rowsInserted, this, &LoggerView::resizeColumns);
  connect(model, &LogModel::rowsInserted, this, &LoggerView::onNewRows);
}
...
void LoggerView::setDoScroll(bool s) { doScroll = s; }

void LoggerView::onNewRows(QModelIndex const &parent, int first, int last) {
  if (doScroll) tableView->scrollToBottom();
...
void LoggerView::saveLines(QModelIndex const &parent, int first, int last) {
  for (int i = first; i <= last; i++) {
    QDateTime const time(
        tableView->model()->index(i, 0, parent).data().toDateTime());
    QString const msg(
        tableView->model()->index(i, 2, parent).data().toString());
...
void LoggerView::pickSaveFile() {
  QString fileName{QFileDialog::getSaveFileName(
      this, tr("Select a file where to append logs into"), "/tmp",
      tr("Log file (*)"), nullptr, QFileDialog::DontConfirmOverwrite)};

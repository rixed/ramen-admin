...
StoragePies::StoragePies(GraphModel *graphModel_, QWidget *parent)
  ...
  QHBoxLayout *modeSelect{new QHBoxLayout};
  {
    ...
    connect(graphModel, &GraphModel::functionAdded, this,
            &StoragePies::updateSumSitesCheckBox);
    connect(graphModel, &GraphModel::functionRemoved, this,
            &StoragePies::updateSumSitesCheckBox);
    ...
    connect(sum, &QCheckBox::stateChanged, this, &StoragePies::refreshChart);
  }
  ...
  QWidget *info{new QWidget(this)};
  {
    ...
    infoLayout->addWidget(new QLabel(tr("Current size:")), 0, 2,
                          Qt::AlignRight);
    infoLayout->addWidget(selectionCurrentInfo, 0, 3, Qt::AlignLeft);

    selectionAllocInfo = new QLabel;
    infoLayout->addWidget(new QLabel(tr("Allocated size:")), 1, 2,
                          Qt::AlignRight);
...
void StoragePies::refreshChart() {
  ...
  for (auto &siteItem : graphModel->sites) {
    QString const &siteName{collapse[0] ? collapsed : siteItem->shared->name};
    Key k0{siteName, collapsed, collapsed};
    for (auto &programItem : siteItem->programs) {
      QString const &progName{collapse[1] ? collapsed
                                          : programItem->shared->name};
      ...
      for (auto &functionItem : programItem->functions) {
        ...
        QString const &funcName{collapse[2] ? collapsed : function->name};
        Key k2{siteName, progName, funcName};
        Values v{function->numArcBytes ? *function->numArcBytes : 0,
  ...
  for (unsigned r = 0; r < 2; r++) {
    radius[r + 2] =
        std::sqrt(2 * radius[r + 1] * radius[r + 1] - radius[r] * radius[r]);
  }
    ...
    if (!collapse[r]) {
      ...
      for (auto const &it : rings[r]) {
        ...
        bool labelVisible{v.forMode(dataMode) >= minValueForLabel &&
                          isLastRing};
        StorageSlice *slice{new StorageSlice(c, labelVisible, k, v, dataMode)};
        ...
        connect(slice, &StorageSlice::clicked, this,
                &StoragePies::toggleSelection);
        pie->append(slice);
      }
      chart->addSeries(pie);
      currentRing++;
...
void StoragePies::displaySelection(Key const &k, Values const &val) {
  selectionSiteLabel->setText(
      k.isValid() ? (k.name[0].isEmpty() ? "ø" : k.name[0]) : "");
  selectionProgLabel->setText(k.isValid() ? k.name[1] : "");
  selectionFuncLabel->setText(k.isValid() ? k.name[2] : "");

  selectionCurrentInfo->setText(val.current >= 0 ? stringOfBytes(val.current)
                                                 : "");
  selectionAllocInfo->setText(val.allocated >= 0 ? stringOfBytes(val.allocated)
                                                 : "");
}

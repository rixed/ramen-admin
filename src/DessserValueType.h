...
struct DessserValueType {
  ...
  virtual std::shared_ptr<RamenType const> columnType(int) const {
    return nullptr;
  }
  ...
  virtual RamenValue *unserialize(uint32_t const *&start, uint32_t const *max,
                                  bool topLevel) const = 0;
...
struct Unknown : DessserValueType {
  ...
  RamenValue *unserialize(uint32_t const *&start, uint32_t const *max,
                          bool topLevel) const;
};
...
struct TVec : DessserValueType {
  ...
  RamenValue *unserialize(uint32_t const *&start, uint32_t const *max,
                          bool topLevel) const;
  // Vectors are displayed in dim columns. Maybe a single one would be
  // preferable?
...
struct TRecord : DessserValueType {
  ...
  std::vector<std::pair<QString const, std::shared_ptr<RamenType const> > >
      fields;
...
struct TSum : DessserValueType {
  ...
  /* Like records, sums are constructed empty; alternatives must be pushed one
   * by one. */
  void append(QString const, std::shared_ptr<RamenType const> const);

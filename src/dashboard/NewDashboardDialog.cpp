...
NewDashboardDialog::NewDashboardDialog(QWidget *parent) : QDialog(parent) {
  ...
  connect(buttonBox, &QDialogButtonBox::accepted, this,
          &NewDashboardDialog::createDashboard);
...
void NewDashboardDialog::createDashboard() {
  if (verbose) qDebug() << "NewDashboardDialog: creating new dashboard...";
  ...
  std::string const placeholder{tr("Sample text widget where you could enter a "
  ...
  if (verbose) qDebug() << "NewDashboardDialog: opening new dashboard...";

  Menu::openDashboard(dash_name);
}

void NewDashboardDialog::clear() { nameEdit->clear(); }

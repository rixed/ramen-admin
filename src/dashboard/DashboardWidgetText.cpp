...
DashboardWidgetText::DashboardWidgetText(DashboardWidgetForm *widgetForm,
                                         QWidget *parent)
    : AtomicWidget(parent) {
   ...
   * of the AtomicForm that may also call hide/show on the buttons
   * individually. Therefore we pack them in a widget and will hide/show that
   * widget instead:
   */
  ...
  if (widgetForm) {
    ...
    connect(widgetForm, &DashboardWidgetForm::changeEnabled, this,
            [buttonBox](bool enabled) { buttonBox->setVisible(enabled); });
  };

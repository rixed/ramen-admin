...
std::string const dashboardNameOfKey(dessser::gen::sync_key::t const &key) {
  if (key.index() == dessser::gen::sync_key::Dashboards) {
    auto const &dashboards{std::get<dessser::gen::sync_key::Dashboards>(key)};
...
int dashboardNumWidgets(std::string const &dash_name) {
  ...
  iterDashboardWidgets(dash_name,
                       [&num](std::shared_ptr<dessser::gen::sync_key::t const>,
                              KValue const &) { num++; });
...
void iterDashboardWidgets(
    ...
    std::function<void(std::shared_ptr<dessser::gen::sync_key::t const>,
                       KValue const &)>
        f) {
  /* Reminder: kvs->map is not sorted. Therefore to map must be scanned first,
   * and the list of keys ordered by indices before calling [f]. */
  std::map<uint32_t, std::pair<std::shared_ptr<dessser::gen::sync_key::t const>,
                               KValue const &> >
      widgets;
  kvs->lock.lock_shared();
  for (std::pair<std::shared_ptr<dessser::gen::sync_key::t const> const,
  ...
  /* Still holding the KVS->map lock so that the callback can assume those
   * dashboard widgets do exist, call [f]: */
  for (auto const &k_v : widgets) {
    std::shared_ptr<dessser::gen::sync_key::t const> const &key{
        k_v.second.first};
...
std::shared_ptr<dessser::gen::sync_key::t> dashboardNextWidget(
    std::string const &dash_name) {
  std::optional<uint32_t> num;

  iterDashboardWidgets(
      dash_name, [&num](std::shared_ptr<dessser::gen::sync_key::t const> k,
                        KValue const &) {

...
void NamesTree::updateNames(dessser::gen::sync_key::t const &key,
  ...
  if (verbose)
    qDebug() << "NamesTree: found" << site << "/ " << programs << "/"
             << function;
  ...
  if (!sourceInfos) {
    if (verbose) qDebug() << "NamesTree: No source info yet for" << infoKey;
    return;
  }

  if (sourceInfos->detail.index() != dessser::gen::source_info::Compiled) {
    if (verbose) qDebug() << "NamesTree:" << infoKey << "not compiled yet";
    return;
  }
  ...
  for (std::shared_ptr<dessser::gen::source_info::compiled_func> const &info :
     ...
     * would come handy, for instance to determine if a field is numeric, or
     * a factor, etc */
...
void NamesTree::deleteNames(dessser::gen::sync_key::t const &key,
                            KValue const &) {
...
NamesSubtree::NamesSubtree(NamesTree const &namesTree,
                           QModelIndex const &newRoot_)
    : NamesTree(namesTree.withSites), newRoot(newRoot_) {
...
QModelIndex NamesSubtree::index(int row, int column,
...
NamesCompleter::NamesCompleter(NamesTree *model, QObject *parent,
                               QModelIndex const &newRoot_)
    : QCompleter(model, parent), newRoot(newRoot_) {
  if (verbose)
...
QString NamesCompleter::pathFromIndex(QModelIndex const &index) const {
  ...
  ConfSubTree *root{
      newRoot.isValid()
          ? static_cast<ConfSubTree *>(QModelIndex(newRoot).internalPointer())
          : nullptr};
  if (verbose) qDebug() << "NamesCompleter: root@" << root << ":" << root->name;

  while (tree->parent && tree->parent->parent && tree->parent != root) {
    tree = tree->parent;
    if (verbose) qDebug() << "NamesCompleter: tree@" << tree;
    ret.prepend('/');

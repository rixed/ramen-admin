...
LoginWin::LoginWin(QString configDir, QWidget *parent)
  ...
  connect(loginWidget, &LoginWidget::submitted, this, &LoginWin::startConnect);
  connect(loginWidget, &LoginWidget::cancelled, this, &LoginWin::exitApp);

  /* As a status bar we will display the error from the confserver in
   * errorMessage and some statistics about the connection. */
  ...
  connect(this, &LoginWin::authenticated, menu, &Menu::upgradeToFull);

  statsTimer = new QTimer(this);
  connect(statsTimer, &QTimer::timeout, this, &LoginWin::refreshStats);
  // Will be started when sync starts
}

LoginWin::~LoginWin() {
  // Since the process is about to exit there is no need to close the client
  // etc.
}

void LoginWin::focusSubmit() { loginWidget->focusSubmit(); }

void LoginWin::startConnect(QString const server, QString const username,
  ...
  connect(client, &ConfClient::knownErrKey, errorMessage, &KErrorMsg::setKey);
  connect(client, &ConfClient::connectionProgressed, this,
          &LoginWin::syncProgress);
  connect(client, &ConfClient::connectionFatalError, this,
          &LoginWin::syncFailed);

  loginWidget->setSubmitStatus("");
}

void LoginWin::exitApp() { QCoreApplication::quit(); }

void LoginWin::refreshStats() {
  QString const statusMsg{tr("%1 (%2) ⥮ %3 (%4)")

...
KArrEditor::KArrEditor(std::shared_ptr<dessser::gen::raql_type::t const> type_,
                       QWidget *parent)
    : AtomicWidget(parent), type(type_) {
  ...
  connect(addButton, &QPushButton::clicked, this, &KArrEditor::addItem);
...
std::shared_ptr<dessser::gen::sync_value::t const> KArrEditor::getValue()
    const {

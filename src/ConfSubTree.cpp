...
ConfSubTree::ConfSubTree(ConfSubTree const &other, ConfSubTree *parent_)
  ...
  if (verbose)
    qDebug() << "ConfSubTree: Copying a ConfSubTree with" << other.count()
             << "children";
...
int ConfSubTree::count() const { return children.size(); }
...
ConfSubTree *ConfSubTree::child(int pos) {
  return const_cast<ConfSubTree *>(
      const_cast<ConfSubTree const *>(this)->child(pos));
}

int ConfSubTree::childNum(ConfSubTree const *child) const {
  if (verbose)
    qDebug() << "ConfSubTree: childNum(" << child->name << ")"
             << "of" << name;

  for (size_t c = 0; c < children.size(); c++) {
    if (children[c]->name == child->name && children[c] != child)
...
void ConfSubTree::dump(QString const &indent) const {
  for (ConfSubTree *c : children) {
    qDebug() << "ConfSubTree:" << indent << c->name
...
ConfSubTree::dump_c(int const indent) const {
  ...
  for (ConfSubTree *c : children) {
    std::cout << "ConfSubTree:" << indent_ << c->name.toStdString()
              << "(parent="
...
ConfSubTree *ConfSubTree::insertAt(int pos, QString const &name,

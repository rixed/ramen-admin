...
KCidrEditor::KCidrEditor(QWidget *parent, bool allow_v4, bool allow_v6)
  ...
  connect(ipEditor, &KIpEditor::inputChanged, this, &KCidrEditor::inputChanged);
  connect(maskEditor, &KIntEditor::inputChanged, this,
          &KCidrEditor::inputChanged);
}

std::shared_ptr<dessser::gen::sync_value::t const> KCidrEditor::getValue()
    const {
  ...
  std::shared_ptr<dessser::gen::sync_value::t const> mask_{
      maskEditor->getValue()};
  ...
  if (allowV4 && allowV6) {
      ...
      case dessser::gen::raql_value::VIp: {
        ...
        if (ip_.index() == dessser::gen::raql_value::v4) {
          ...
          return makeRamenValue(std::make_shared<dessser::gen::raql_value::t>(
              ...
              std::in_place_index<dessser::gen::raql_value::v4>, ipv4, mask));
        } else {
          Q_ASSERT(ip_.index() == dessser::gen::raql_value::v6);
          ipv6 = std::get<dessser::gen::raql_value::v6>(ip_);
          return makeRamenValue(std::make_shared<dessser::gen::raql_value::t>(
              std::in_place_index<dessser::gen::raql_value::VCidr>,
              std::in_place_index<dessser::gen::raql_value::v6>, ipv6, mask));
        }
      }
    ...
    ipv4 = std::get<dessser::gen::raql_value::VIpv4>(*ip);
    return makeRamenValue(std::make_shared<dessser::gen::raql_value::t>(
        std::in_place_index<dessser::gen::raql_value::VCidrv4>, ipv4, mask));
  } else if (allowV6) {
    // returns a Cidrv6 from the Ipv6
    ipv6 = std::get<dessser::gen::raql_value::VIpv6>(*ip);
    return makeRamenValue(std::make_shared<dessser::gen::raql_value::t>(
        std::in_place_index<dessser::gen::raql_value::VCidrv6>, ipv6, mask));
  } else {
    return nullptr;
  }
}

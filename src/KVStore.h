...
struct HashKey {
  std::size_t operator()(
      std::shared_ptr<dessser::gen::sync_key::t const> k) const noexcept {
...
struct EqualKey {
  ...
  bool operator()(std::shared_ptr<dessser::gen::sync_key::t const> const &k1,
                  std::shared_ptr<dessser::gen::sync_key::t const> const &k2)
      const noexcept {
    return (*k1 == *k2);
  }
};
...
class KVStore : public QObject {
 ...
 public:
  ...
  /* Key to value bindings are stored in a single hashtable, with some
   * additional views maintained for performance reasons. These are actually
   * maintained in sync by ConfClient. All of those data containers are
  ...
  std::unordered_map<std::shared_ptr<dessser::gen::sync_key::t const>, KValue,
                     HashKey, EqualKey>
      map;
  /* Also the set of all existing incident identifiers, once for every
   * occurrence for easy deletion. Also protected by [lock]: */
  std::multiset<std::string> incident_ids;
  /* Similarly, a map of dialog identifiers per incident identifiers, also
   * multimap for easy deletion, and also protected by [lock]: */
  std::multimap<std::string, std::string> dialog_ids;
  /* Finally, a multimap of logs identifiers per incident identifiers. */
  std::multimap<std::string, std::shared_ptr<dessser::gen::sync_key::t const> >
      incident_logs;
...
/* There is a single one, created by main, so that QObjects can connect to it
 * even before connection to confserver is established: */

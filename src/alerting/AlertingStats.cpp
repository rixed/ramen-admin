...
AlertingStats::AlertingStats(QWidget *parent) : QWidget(parent) {
  ...
  connect(timer, &QTimer::timeout, this,
          QOverload<>::of(&AlertingStats::updateStats));

  connect(kvs.get(), &KVStore::keyChanged, this, &AlertingStats::onChange);

  timer->start(20000);
}

void AlertingStats::updateStats() {
  if (verbose) qDebug() << "AlertingStats::updateStats: dirty=" << dirty;
  ...
  std::function<void(
      ...
      updateNotifDates{
          [&lastNotification, &oldestNotification](
              std::shared_ptr<
                  dessser::gen::alerting_notification::t const> const notif) {
            lastNotification =
                std::max<double>(lastNotification, notif->sent_time);
            oldestNotification =
                std::min<double>(oldestNotification, notif->sent_time);
          }};
  ...
  iterIncidents([&numFiringIncidents, &numIncidents, &numDialogs,
    ...
    std::shared_ptr<dessser::gen::alerting_notification::t const> const
        firstStartNotif{getIncidentNotif(
            incidentId,
    ...
    std::shared_ptr<dessser::gen::alerting_notification::t const> const
        lastChangeNotif{getIncidentNotif(
    ...
    iterDialogs(incidentId, [&numDialogs, &lastDeliveryAttempt, &nextSchedule,
                             &nextSend,
  ...
  if (numDialogs[dessser::gen::alerting_delivery_status::StartToBeSent] > 0)
    numDialogsStr.append(
        QString::number(
            numDialogs[dessser::gen::alerting_delivery_status::StartToBeSent]) +
        " alerts to be sent");
  if (numDialogs
          [dessser::gen::alerting_delivery_status::StartToBeSentThenStopped] >
      0)
    numDialogsStr.append(
        QString::number(numDialogs[dessser::gen::alerting_delivery_status::
  ...
  if (numDialogs[dessser::gen::alerting_delivery_status::StartSent] > 0)
    numDialogsStr.append(
        QString::number(
            numDialogs[dessser::gen::alerting_delivery_status::StartSent]) +
        " alerts sent");
  if (numDialogs[dessser::gen::alerting_delivery_status::StartAcked] > 0)
    numDialogsStr.append(
        QString::number(
            numDialogs[dessser::gen::alerting_delivery_status::StartAcked]) +
        " alerts acked");
  if (numDialogs[dessser::gen::alerting_delivery_status::StopToBeSent] > 0)
    numDialogsStr.append(
        QString::number(
            numDialogs[dessser::gen::alerting_delivery_status::StopToBeSent]) +
        " recovery to be sent");
  if (numDialogs[dessser::gen::alerting_delivery_status::StopSent] > 0)
    numDialogsStr.append(
        QString::number(
            numDialogs[dessser::gen::alerting_delivery_status::StopSent]) +
        " recovery sent");
...
bool AlertingStats::isMyKey(dessser::gen::sync_key::t const &k) const {
  return k.index() == dessser::gen::sync_key::Teams ||
